...
# Database Cleaner by RIR18
    async def config_dict(self):
        if self._return:
            return
        await self.db.settings.config.find_one({"_id": TgClient.ID})
        self._conn.close
    ...
    async def clean_config(self, dict_):
        if self._return:
            return
        await self.db.settings.config.delete_one({'_id': TgClient.ID})
        self._conn.close
    ...
    async def clean_private_files(self, dict_):
        if self._return:
            return
        await self.db.settings.files.delete_one({'_id': TgClient.ID})
        self._conn.close
