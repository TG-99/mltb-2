...
from bot import bot, botStartTime, LOGGER, Interval, DATABASE_URL, QbInterval, INCOMPLETE_TASK_NOTIFIER, scheduler, config_dict, alive
...
from .modules import authorize, clone, gd_count, gd_delete, cancel_mirror, gd_search, mirror_leech, status, torrent_search, torrent_select, ytdlp, rss, shell, eval, users_settings, bot_settings, cleandb
...
async def start(client, message):
    buttons = ButtonMaker()
    buttons.ubutton(
        "GD Index", "https://gdrive.rir18.workers.dev")
...
async def restart(_, message):
    ...
    proc1 = await create_subprocess_exec('pkill', '-9', '-f', 'gunicorn|mltb-a|mltb-q|mltb-f|nginx|rclone')
...
/{BotCommands.CleanDBCommand}:Clean Database.
'''
